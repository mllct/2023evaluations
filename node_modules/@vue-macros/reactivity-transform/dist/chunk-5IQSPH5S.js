...
var transformCheckRE = /\W\$(?:\$|ref|computed|shallowRef|toRef|customRef)?\s*([(<])/;
...
`
  ...
  walk(ast, {
    enter(node, parent) {
      ...
      if (_compilercore.isFunctionType.call(void 0, node)) {
      ...
      if (node.type === "BlockStatement" && !_compilercore.isFunctionType.call(void 0, parent)) {
        scopeStack.push(currentScope = {});
        walkScope(node);
        return;
      }
      ...
      if (node.type === "CallExpression" && node.callee.type === "Identifier") {
        ...
        if (escapeSymbol && getCurrentScope()[escapeSymbol] === void 0 && callee === escapeSymbol) {
          ...
          if ((parent2 == null ? void 0 : parent2.type) === "ExpressionStatement") {
    ...
    leave(node, parent) {
      ...
      if (node.type === "BlockStatement" && !_compilercore.isFunctionType.call(void 0, parent) || _compilercore.isFunctionType.call(void 0, node)) {
        scopeStack.pop();
...
var code_default = "export function createPropsRestProxy(props, excludedKeys) {\n  const ret = {}\n  for (const key in props) {\n    if (!excludedKeys.includes(key)) {\n      Object.defineProperty(ret, key, {\n        enumerable: true,\n        get: () => props[key],\n      })\n    }\n  }\n  return ret\n}\n";

// src/core/helper/index.ts
var helperId = "/vue2-reactivity-transform-helper";
...
function transformVueSFC(code, id) {
  ...
  function processDefineProps(node) {
    ...
    if (!decl || decl.id.type !== "ObjectPattern")
      return;
    ...
    for (const prop of decl.id.properties) {
      if (prop.type === "ObjectProperty") {
        const propKey = _common.resolveObjectKey.call(void 0, prop.key, prop.computed, false);
        if (!propKey) {
          throw new SyntaxError(
...
function addHelpers(s, offset, helpers) {
  if (helpers.length === 0)
    return;
  s.prependLeft(
    offset,
...
function resolveOption(options, framework) {
  return {
    include: [_common.REGEX_SRC_FILE, _common.REGEX_VUE_SFC, _common.REGEX_SETUP_SFC].concat(
      framework === "webpack" ? _common.REGEX_VUE_SUB : []
    ),
